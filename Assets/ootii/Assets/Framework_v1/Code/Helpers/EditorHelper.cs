...
namespace com.ootii.Helpers
{
    public class EditorHelper
    {
        ...
        // CDL 07/26/2018 - Renamed from "LastPath" to better differentiate from LastFolderPath 
        public static string LastFilePath = "";

        // CDL 07/26/2018 - for storing the last result from FolderSelect
        ...
        // CDL 08/03/2018 - needed this when renaming an asset file
        /// <summary>
        /// Simplified way to create a text field that doesn't change its return value until the user has pressed
        /// enter or focus is moved away from the field
        /// </summary>
        /// <returns></returns>
        public static bool DelayedTextField(string rTitle, string rTip, string rValue, UnityEngine.Object rRecorder = null, float rMinWidth = 0)
        {
            EditorGUI.BeginChangeCheck();

            if (rMinWidth <= 0)
            {
                FieldStringValue = EditorGUILayout.DelayedTextField(new GUIContent(rTitle, rTip), rValue);
            }
            else
            {
                FieldStringValue = EditorGUILayout.DelayedTextField(new GUIContent(rTitle, rTip), rValue, GUILayout.MinWidth(rMinWidth));
            }
        ...
        public static bool Vector3Field(string rTitle, string rTip, Vector3 rValue, UnityEngine.Object rRecorder = null)
        {
            ...
            EditorGUI.BeginChangeCheck();
            FieldVector3Value.x = EditorGUILayout.FloatField(FieldVector3Value.x, GUILayout.MinWidth(20));
            if (EditorGUI.EndChangeCheck())
            {
                if (rRecorder != null) { Undo.RecordObject(rRecorder, "Set " + rTitle); }
                lIsDirty = true;
            ...
            EditorGUI.BeginChangeCheck();
            FieldVector3Value.y = EditorGUILayout.FloatField(FieldVector3Value.y, GUILayout.MinWidth(20));
            if (EditorGUI.EndChangeCheck())
            {
                if (rRecorder != null) { Undo.RecordObject(rRecorder, "Set " + rTitle); }
                lIsDirty = true;
            ...
            EditorGUI.BeginChangeCheck();
            FieldVector3Value.z = EditorGUILayout.FloatField(FieldVector3Value.z, GUILayout.MinWidth(20));
            if (EditorGUI.EndChangeCheck())
            {
                if (rRecorder != null) { Undo.RecordObject(rRecorder, "Set " + rTitle); }
                lIsDirty = true;
        ...
        public static string FileSelect(GUIContent rLabel, string rValue, string rFileTypes)
        {
            ...
            {
                lNewValue = EditorUtility.OpenFilePanel("Select the file", LastFilePath, rFileTypes);
                if (lNewValue.Length != 0)
                {
                    LastFilePath = lNewValue;
        ...
        // CDL 07/26/2018 - added to support selecting the Animations folder path from a motion pack inspector
        /// <summary>
        /// Shows a folder selection field and button. The field has the label placed above the input controls,
        /// as paths are frequently fairly long and it's easier to read the path this way.
        /// </summary>
        /// <param name="rLabel"></param>
        /// <param name="rValue"></param>
        /// <returns></returns>
        public static string FolderSelect(GUIContent rLabel, string rValue, out bool rChanged)
        {
            rChanged = false;
            try
            {
                EditorGUILayout.BeginVertical();
                EditorGUILayout.LabelField(rLabel);

                EditorGUILayout.BeginHorizontal();
                string lNewValue = EditorGUILayout.TextField(string.Empty, rValue);
                if (lNewValue != rValue)
                {
                    rChanged = true;
                    rValue = lNewValue;
                ...
                if (GUILayout.Button(new GUIContent("...", "Select folder"), EditorStyles.miniButton, GUILayout.Width(20)))
                {
                    lNewValue = EditorUtility.OpenFolderPanel("Select the animations folder", LastFolderPath, "");
                    if (lNewValue.Length != 0)
                    {
                        rChanged = true;
                        rValue = GetRelativeDataPath(lNewValue);
                        LastFolderPath = rValue;
                    }
                }
                EditorGUILayout.EndHorizontal();
            }
        ...
        // CDL 07/26/2018 - if we're saving a file or folder path (in a ScriptableObject, for example),
        // we don't want to save the full path, as the settings should be portable
        /// <summary>
        /// Convert an absolute file path (i.e. "C:/My Projects/My Game/Assets/Animations/") to a 
        /// relative path starting from "Assets" (i.e. "Assets/Animations")
        /// </summary>
        /// <param name="rAbsolutePath"></param>
        /// <returns></returns>
        ...
        /// Renders the inspector title for our asset
        /// </summary>
        public static void DrawInspectorTitle(string rTitle, GUIStyle rLabelStyle = null)
        {
            ...
            EditorGUILayout.LabelField(rTitle, rLabelStyle == null ? EditorHelper.Title : rLabelStyle);
        ...
        // CDL 07/06/2018 - added overload to eliminate having to include MessageType.None on every help box
        public static void DrawInspectorDescription(string rDescription)
        {
            DrawInspectorDescription(rDescription, MessageType.None);
        }
        ...
        // CDL 08/03/2018 - for Editor Windows
        public static void DrawWindowHeader(string rTitle, string rDescription = "")
        {
            GUILayout.Space(5);
            ...
            EditorGUILayout.LabelField(rTitle, WindowTitle,  GUILayout.ExpandWidth(true));//GUILayout.MinWidth(400f),    
            ...
            if (!string.IsNullOrEmpty(rDescription))
            {
                ...
                //int lFontSize = lStyle.fontSize;
                //Debug.Log("Font size = " + lFontSize);
                //lStyle.fontSize++;

                //EditorGUILayout.BeginVertical(GUI.skin.box);
                //EditorGUILayout.HelpBox(rDescription, MessageType.None);
                EditorGUILayout.LabelField(rDescription, WrapLabel);
                //lStyle.fontSize = lFontSize;
        ...
        // CDL 07/13/2018 - added this to use in the QuickStart editor window
        /// <summary>
        /// Display a foldout section header
        /// </summary>
        /// <param name="rTitle"></param>
        /// <param name="rEnabled"></param>
        /// <returns></returns>
        public static bool DrawFoldoutHeader(string rTitle, bool rEnabled)
        {
            bool lClicked = false;
            try
            {
                GUILayout.BeginHorizontal();
                if (GUILayout.Button(new GUIContent(rTitle), EditorStyles.boldLabel))
                {
                    lClicked = true;
                }
                GUILayout.FlexibleSpace();

                if (GUILayout.Button(new GUIContent(rEnabled ? "-" : "+"), EditorStyles.boldLabel))
                {
                    lClicked = true;
                }
            }
            ...
            return lClicked ? !rEnabled : rEnabled;
        }

        public static bool DrawFoldoutHeader(Rect rRect, string rTitle, bool rEnabled)
        {
            bool lClicked = false;
            try
            {
                GUILayout.BeginArea(rRect);
                lClicked = DrawFoldoutHeader(rTitle, rEnabled);
        ...
        /// <summary>
        /// Draw the quick-set "Clear All" and "Select All" buttons for a group of settings
        /// </summary>
        /// <param name="rSetOptionsAction">Delegate function that sets the appropriate options</param>
        public static void DrawQuickSetButtons(Action<bool> rSetOptionsAction)
        {
            ...
            try
            {
                ...
                if (GUILayout.Button("Clear All", EditorStyles.miniButton, GUILayout.Width(65)))
                {
                    if (rSetOptionsAction != null) rSetOptionsAction(false);
                }

                if (GUILayout.Button("Select All", EditorStyles.miniButton, GUILayout.Width(65)))
                {
                    if (rSetOptionsAction != null) rSetOptionsAction(true);
                }
            }
            finally
            {
        ...
        /// <summary>
        /// Draw the quick-set "Clear All" button for a group of settings
        /// </summary>
        /// <param name="rSetOptionsAction">Delegate function that sets the appropriate options</param>
        public static void DrawQuickClearButton(Action rSetOptionsAction)
        {
            EditorGUILayout.Separator();
            ...
            try
            {
                ...
                if (GUILayout.Button("Clear All", EditorStyles.miniButton, GUILayout.Width(65)))
                {
                    if (rSetOptionsAction != null) rSetOptionsAction();
                }              
            }
            finally
            {
        ...
        // CDL 08/03/2018 - to streamline some of the more complex inspectors a bit
        /// <summary>
        /// Draw an standard Inspector block with a foldout header
        /// </summary>
        /// <param name="rTitle"></param>
        /// <param name="rDescription"></param>
        /// <param name="rDrawContentsAction"></param>
        /// <param name="rFoldoutState"></param>
        /// <param name="rDisabled"></param>
        public static void DrawInspectorBlock(string rTitle, string rDescription, Action rDrawContentsAction, ref bool rFoldoutState, bool rDisabled = false)
        {
            rFoldoutState = DrawFoldoutHeader(rTitle, rFoldoutState);
        ...
        // CDL 08/03/2018 - to streamline some of the more complex inspectors a bit
        /// <summary>
        /// Draw a standard Inspector block (no foldout)
        /// </summary>
        /// <param name="rTitle"></param>
        /// <param name="rDescription"></param>
        /// <param name="rDrawContentsAction"></param>
        /// <param name="rDisabled"></param>
        public static void DrawInspectorBlock(string rTitle, string rDescription, Action rDrawContentsAction, bool rDisabled = false)
        {
        ...
        // CDL 08/03/2018 - for EditorWindows
        /// <summary>
        /// Draw a standardized Editor Window footer
        /// </summary>
        /// <param name="rDrawContents"></param>
        /// <param name="rDisabled"></param>
        /// <param name="rAlignment"></param>
        public static void DrawWindowFooter(Action rDrawContents, bool rDisabled, TextAlignment rAlignment = TextAlignment.Right)
        {
            GUILayout.FlexibleSpace();
            ...
            EditorGUILayout.BeginHorizontal();
            if (rAlignment == TextAlignment.Center || rAlignment == TextAlignment.Right)
            {
                GUILayout.FlexibleSpace();
            }
            ...
            if (rAlignment == TextAlignment.Center || rAlignment == TextAlignment.Left)
            {
                GUILayout.FlexibleSpace();
            }            
            EditorGUILayout.EndHorizontal();
        ...
        public static GUIStyle ReadOnlyLabel
        {
            get
            {
                if (mReadOnlyLabel == null)
                {
                    ...
                    Texture2D lTexture = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "ClearBox" : "ClearBox");
        ...
        public static GUIStyle GoalAtomic
        {
            get
            {
                if (mGoalAtomic == null)
                {
                    ...
                    Texture2D lTexture = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "GoalAtomicSimple" : "GoalAtomicSimple");
        ...
        public static GUIStyle GoalComposite
        {
            get
            {
                if (mGoalAtomic == null)
                {
                    ...
                    Texture2D lTexture = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "GoalCompositeSimple" : "GoalCompositeSimple");
        ...
        public static GUIStyle SmallTitle
        {
            get
            {
                if (mSmallTitle == null)
                {
                    ...
                    mSmallTitle.normal.textColor = (EditorGUIUtility.isProSkin ? new Color(0.7f, 0.7f, 0.7f) : Color.white);
        ...
        public static GUIStyle Title
        {
            get
            {
                if (mTitle == null)
                {
                    ...
                    mTitle.normal.textColor = (EditorGUIUtility.isProSkin ? new Color(0.7f, 0.7f, 0.7f) : Color.white);
        ...
        /// </summary>
        private static GUIStyle mWindowTitle = null;
        public static GUIStyle WindowTitle
        {
            get
            {
                if (mWindowTitle == null)
                {
                    ...
                    mWindowTitle = new GUIStyle(GUI.skin.label)
                    {
                        alignment = TextAnchor.LowerLeft,                        
                        font = lFont,
                        fontSize = 14,
                        fontStyle = FontStyle.Bold,
                        normal = {textColor = (EditorGUIUtility.isProSkin ? new Color(0.7f, 0.7f, 0.7f) : Color.white)},
                        fixedHeight = 22f,
                        fixedWidth = 500f,
        ...
        public static GUIStyle SmallBox
        {
            get
            {
                if (mSmallBox == null)
                {
                    //Texture2D lTexture = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "Editor/TitleBox_pro" : "Editor/TitleBoxLight");
                    Texture2D lTexture = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "TitleBox_pro" : "TitleBoxLight");
        ...
        public static GUIStyle TitleBox
        {
            get
            {
                if (mTitleBox == null)
                {
                    //Texture2D lTexture = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "Editor/TitleBox_pro" : "Editor/TitleBox");
                    Texture2D lTexture = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "TitleBox_pro" : "TitleBox");
        ...
        public static GUIStyle ThinGroupBox
        {
            get
            {
                if (mThinGroupBox == null)
                {
                    ...
                    Texture2D lTexture = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "GroupBox_pro" : "GroupBox");
        ...
        public static GUIStyle RedBox
        {
            get
            {
                if (mRedBox == null)
                {
                    ...
                    Texture2D lTexture = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "RedBox_pro" : "RedBox");
        ...
        public static GUIStyle GreenBox
        {
            get
            {
                if (mGreenBox == null)
                {
                    ...
                    Texture2D lTexture = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "GreenBox_pro" : "GreenBox");
        ...
        public static GUIStyle Line
        {
            get
            {
                if (mLine == null)
                {
                    ...
                    Texture2D lTexture = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "Line_pro" : "Line");
        ...
        public static GUIStyle OrangeGearButton
        {
            get
            {
                if (mOrangeGearButton == null)
                {
                    ...
                    mOrangeGearButton.normal.background = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "GearButtonOrange_pro" : "GearButtonOrange");
        ...
        public static GUIStyle BlueGearButton
        {
            get
            {
                if (mBlueGearButton == null)
                {
                    ...
                    mBlueGearButton.normal.background = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "GearButtonBlue_pro" : "GearButtonBlue");
        ...
        public static GUIStyle GraySelectButton
        {
            get
            {
                if (mGraySelectButton == null)
                {
                    ...
                    mGraySelectButton.normal.background = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "SelectButtonGray" : "SelectButtonGray");
        ...
        public static GUIStyle GrayAddButton
        {
            get
            {
                if (mGrayAddButton == null)
                {
                    ...
                    mGrayAddButton.normal.background = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "AddButtonGray" : "AddButtonGray");
        ...
        public static GUIStyle GrayXButton
        {
            get
            {
                if (mGrayXButton == null)
                {
                    ...
                    mGrayXButton.normal.background = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "DeleteButtonGray" : "DeleteButtonGray");
        ...
        public static GUIStyle BlueSelectButton
        {
            get
            {
                if (mBlueSelectButton == null)
                {
                    ...
                    mBlueSelectButton.normal.background = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "SelectButton" : "SelectButton");
        ...
        public static GUIStyle BlueAddButton
        {
            get
            {
                if (mBlueAddButton == null)
                {
                    ...
                    mBlueAddButton.normal.background = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "AddButtonBlue" : "AddButtonBlue");
        ...
        public static GUIStyle BlueXButton
        {
            get
            {
                if (mBlueXButton == null)
                {
                    ...
                    mBlueXButton.normal.background = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "DeleteButtonBlue" : "DeleteButtonBlue");
        ...
        public static GUIStyle RedXButton
        {
            get
            {
                if (mRedXButton == null)
                {
                    ...
                    mRedXButton.normal.background = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "DeleteButton" : "DeleteButton");
        ...
        public static GUIStyle LongButtonGreen
        {
            get
            {
                if (mLongButtonGreen == null)
                {
                    ...
                    mLongButtonGreen.normal.background = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "LongButtonGreen" : "LongButtonGreen");
        ...
        public static GUIStyle SelectedLabel
        {
            get
            {
                if (mSelectedLabel == null)
                {
                    ...
                    Texture2D lTexture = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "BlueSelected" : "BlueSelected");

                    mSelectedLabel = new GUIStyle(GUI.skin.label);
                    mSelectedLabel.normal.textColor = (EditorGUIUtility.isProSkin ? new Color(0.7f, 0.7f, 0.7f) : Color.white);
        ...
        private static GUIStyle ootiiIcon
        {
            get
            {
                if (mootiiIcon == null)
                {
                    ...
                    Texture2D lTexture = Resources.Load<Texture2D>(EditorGUIUtility.isProSkin ? "ootii_Icon" : "ootii_Icon");

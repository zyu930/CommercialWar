...
namespace com.ootii.Geometry
{
    ...
    public static class QuaternionExt
    {
        ...
        ///// <summary>
        ///// Creates a quaternion that represents the rotation required to turn the
        ///// original quaternion to the specified quaternion. This version of the
...
#if (UNITY_2017 || UNITY_2018_1)

        // Unity has added this function in Unity 2018.2. So, we don't need this
        // any more.
        ...
        /// represents the opposite angular displacement since the
        /// vector is reversed.
        /// </summary>
        /// <param name="rThis"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Decomposes the quaternion into a twist (around the axis) and a swing (rotation) around a vector perpendicular
        /// to the axis.
        /// 
        /// To rebuild, use:
        /// rThis = rSwing * rTwist
        ...
        {
            // We're trying to account for when the value is serialized as '12,789.12,789.12,789' as opposed to the
            // US serializing as '12.789,12.789,12.789'
            string lDelimiterTest = (rString.IndexOf(' ') >= 0 ? rString.Substring(0, rString.IndexOf(' ')) : rString);

            char lDelimiter = ',';
            int lCommaIndex = lDelimiterTest.IndexOf(',');
            int lPeriodIndex = lDelimiterTest.IndexOf('.');

            if (lPeriodIndex < 0) { lDelimiter = ','; }
            if (lCommaIndex < 0) { lDelimiter = '.'; }
            else if (lCommaIndex < lPeriodIndex) { lDelimiter = '.'; }

            string[] lTemp = rString.Substring(1, rString.Length - 2).Split(lDelimiter);
            if (lTemp.Length != 4) { return rThis; }

            float.TryParse(lTemp[0], NumberStyles.Float, CultureInfo.InvariantCulture, out rThis.x);
            float.TryParse(lTemp[1], NumberStyles.Float, CultureInfo.InvariantCulture, out rThis.y);
            float.TryParse(lTemp[2], NumberStyles.Float, CultureInfo.InvariantCulture, out rThis.z);
            float.TryParse(lTemp[3], NumberStyles.Float, CultureInfo.InvariantCulture, out rThis.w);
            return rThis;
        }
    }
}